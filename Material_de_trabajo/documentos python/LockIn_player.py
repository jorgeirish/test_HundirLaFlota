...
def LockIn(): 
    ...
    def colocar_barcos(barco):
        ...
        while True:
            ...
            coors_posN = tablero_player[fila:fila - eslora:-1, col]
            coors_posE = tablero_player[fila, col: col + eslora]
            coors_posS = tablero_player[fila:fila + eslora, col]
            coors_posO = tablero_player[fila, col: col - eslora:-1]
    
            # Comprobamos si esas posiciones son validas
            # Orientacion Norte
            if orient == 'N' and 0 <= fila - eslora < 10 and 'O' not in coors_posN:
                tablero_player[fila:fila - eslora:-1, col] = 'O'
                break

            # Orientacion Este
            elif orient == 'E' and 0 <= col + eslora < 10 and 'O' not in coors_posE:
                tablero_player[fila, col: col + eslora] = 'O'
                break

            # Orientacion Sur
            elif orient == 'S' and 0 <= fila + eslora < 10 and 'O' not in coors_posS:
                tablero_player[fila:fila + eslora, col] = 'O'
                break

            # Orientacion Oeste
            elif orient == 'O' and 0 <= col - eslora < 10 and 'O' not in coors_posO:
                tablero_player[fila, col: col - eslora:-1] = 'O'
                break
